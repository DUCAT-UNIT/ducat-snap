 ...
 * @param expectedValidatorChainNetwork - Exact reviewed validator contract identity.
...
export async function verifyDeploymentEndpointIdentity(
  expectedValidatorChainNetwork: string,
  ...
  const chainNetwork = body && typeof body === 'object' && !Array.isArray(body) && 'chain_network' in body &&
    ...
    : '';
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(expectedValidatorChainNetwork)) {
    throw new Error('expected validator chain_network is invalid');
  }
  if (chainNetwork !== expectedValidatorChainNetwork) {
    throw new Error(`validator endpoint did not report ${expectedValidatorChainNetwork}`);
  }
}
