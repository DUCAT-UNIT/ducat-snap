...
function setSnapMock(dialogResult = true, selectedNetwork: 'mutinynet' | 'mainnet' = 'mutinynet') {
  ...
  const request = jest.fn(async ({ method, params }: {
    ...
    if (method === 'snap_getBip32Entropy') {
      ...
      return { privateKey: node.privateKey.toString('hex'), chainCode: node.chainCode.toString('hex') };
    }
...
describe('unprompted signing — dev build (flag on)', () => {
  ...
  it('signs a Mutinynet PSBT WITHOUT showing the confirmation dialog', async () => {
  ...
  it('rejects removed alpha-mainnet identity before every wallet effect', async () => {
    const { handleRpcRequest } = loadRpcWithFlag('true');
    const request = setSnapMock(true, 'mainnet');
    const { keySet, psbt } = makeSignablePsbt(100_000, 4);

    await expect(handleRpcRequest(DEV_ORIGIN, {
      method: 'ducat_signPsbtUnprompted',
      params: { network: 'alpha-mainnet', psbt, signInputs: { [keySet.record.sats.address]: [0] } },
    })).rejects.toMatchObject({
      code: 'INVALID_NETWORK',
      details: { requestedNetwork: 'alpha-mainnet' },
    });
    expect(request).not.toHaveBeenCalled();
  });
