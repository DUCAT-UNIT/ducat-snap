...
import { DUCAT_DERIVATION_SCHEME, p2trAccount, p2wpkhAccount, toXOnly } from '../accounts';
...
function setSnapMock(dialogResult = true, initialState: unknown = null): SnapRequestMock {
  let managedState: unknown = initialState && typeof initialState === 'object' && !Array.isArray(initialState)
    ? ('selectedNetwork' in initialState
      ? initialState
      : { ...initialState, selectedNetwork: 'mutinynet' })
    : { recentActions: [], selectedNetwork: 'mutinynet' };
...
function makeImportedPsbt(value: number, seed: number, account: PrivateKeyOverrideRecord) {
  const imported = p2wpkhAccount('mutinynet', Buffer.from(account.sats.pubkey, 'hex'));
  const psbt = new Psbt({ network: bitcoinNetwork('signet') });
...
function makeExternalPsbt(value: number, seed: number) {
  const keySet = testKeySet();
  const external = deriveAccountSetFromBaseNodes('mutinynet', testNode(7), testNode(8));
  const psbt = new Psbt({ network: bitcoinNetwork('signet') });
...
function makeThreeRolePsbt({ outputValue = 89_000, runesValue = 30_000 } = {}) {
  const keySet = testKeySet();
  const redeemScript = btcScript.compile([
    keySet.vaultInternalPubkey,
    opcodes.OP_CHECKSIGVERIFY,
    Buffer.from(DUCAT_GUARDIAN_PUBKEYS.mutinynet[0], 'hex'),
    opcodes.OP_CHECKSIG,
  ]);
  const vaultPayment = payments.p2tr({
    internalPubkey: UNSPENDABLE_TAPROOT_KEY,
    network: bitcoinNetwork('signet'),
    redeem: { output: redeemScript, redeemVersion: 0xc0 },
    scriptTree: { output: redeemScript },
  });

  if (!vaultPayment.output || !vaultPayment.witness?.length) {
    throw new Error('Failed to build the three-role vault fixture.');
  }
  ...
  psbt.addInput({
    hash: '71'.repeat(32),
    index: 0,
    witnessUtxo: { script: keySet.satsOutputScript, value: 30_000 },
  });
  psbt.addInput({
    hash: '72'.repeat(32),
    index: 0,
    tapInternalKey: keySet.runesInternalPubkey,
    witnessUtxo: { script: keySet.runesOutputScript, value: runesValue },
  });
  psbt.addInput({
    hash: '73'.repeat(32),
    index: 0,
    tapLeafScript: [{
      controlBlock: vaultPayment.witness[vaultPayment.witness.length - 1],
      leafVersion: 0xc0,
      script: redeemScript,
    }],
    witnessUtxo: { script: vaultPayment.output, value: 30_000 },
  });
  psbt.addOutput({ address: keySet.record.sats.address, value: outputValue });
  ...
  return {
    ...
    signInputs: {
      ...
      [keySet.record.runes.address]: [1],
      [keySet.record.vault.address]: [2],
    },
  };
}

function copyThreeRoleSignatures(unsigned: Psbt, signed: Psbt): Psbt {
  unsigned.updateInput(0, { partialSig: signed.data.inputs[0].partialSig });
  unsigned.updateInput(1, { tapKeySig: signed.data.inputs[1].tapKeySig });
  unsigned.updateInput(2, { tapScriptSig: signed.data.inputs[2].tapScriptSig });
  return unsigned;
}

const signatureValidator = (pubkey: Buffer, hash: Buffer, signature: Buffer): boolean =>
  pubkey.length === 32 ? ecc.verifySchnorr(hash, pubkey, signature) : ecc.verify(hash, pubkey, signature);
...
describe('RPC router', () => {
  it('uses bundled network profiles for Snap network data', () => {
    expect(networkProfile('mainnet')).toEqual(expect.objectContaining({
      label: 'Mainnet (Alpha contract)',
      expected_validator_chain_network: 'alpha-mainnet',
      validator_base_url: 'https://validator-mainnet.alpha.ducatprotocol.com',
    }));
  ...
  it('derives exact origins, profiles, and capabilities for a dev build', async () => {
    ...
    try {
      jest.isolateModules(() => {
        ...
        expect(developmentProfiles().map((profile) => profile.id)).toEqual(['regtest', 'mainnet', 'mutinynet']);
      });
    } finally {
    ...
    await expect(developmentRpc.handleRpcRequest('http://localhost:3000', {
      ...
      networks: ['regtest', 'mainnet', 'mutinynet'],
  ...
  it('refreshes wallet inventory at the RPC boundary after settlement', async () => {
    setSnapMock();
    (getWalletInventory as jest.Mock).mockClear();

    await handleRpcRequest(ORIGIN, {
      method: 'ducat_getWalletInventory',
      params: { network: 'mutinynet' },
    });

    expect(getWalletInventory).toHaveBeenCalledWith('mutinynet', { fresh: true });
  });

  it('rejects unauthorized origins before requesting entropy', async () => {
    const request = setSnapMock();

    await expect(handleRpcRequest('https://evil.example', { method: 'ducat_getAccounts', params: { network: 'mutinynet' } })).rejects.toMatchObject({
  ...
  it.each(['alpha-mainnet', 'signet', 'testnet4'] as const)(
    'rejects removed wallet deployment %s before side effects',
    async (deployment) => {
      const originalFetch = globalThis.fetch;
      const fetchMock = jest.fn();
      globalThis.fetch = fetchMock as typeof fetch;
      try {
        for (const method of REGISTERED_NETWORK_METHODS) {
          ...
          await expect(handleRpcRequest(ORIGIN, {
            ...
            details: { requestedNetwork: deployment },
          });
          expect(request).not.toHaveBeenCalled();
          expect(fetchMock).not.toHaveBeenCalled();
        }
  ...
  it('development preserves mainnet state and allows an explicitly prompted switch back', async () => {
    ...
    await expect(development.handleRpcRequest(DEV_ORIGIN, {
      ...
      params: { network: 'mutinynet' },
    })).resolves.toEqual({ network: 'mutinynet', changed: true });
  ...
  it('returns derived Ducat account records', async () => {
    ...
    expect(request.mock.calls.filter(([arg]) => arg.method === 'snap_getBip32Entropy').map(([arg]) => arg.params)).toEqual([
      { path: ['m', "84'", "1'", "0'", '0', '0'], curve: 'secp256k1' },
      { path: ['m', "86'", "1'", "0'", '0', '0'], curve: 'secp256k1' },
      { path: ['m', "86'", "1'", "0'", '2', '0'], curve: 'secp256k1' },
    ]);
  });
  ...
  it.each([
    ...
    setSnapMock();
    await expect(handleRpcRequest(ORIGIN, { method, params: { network: 'mutinynet' } })).rejects.toMatchObject({ code: 'METHOD_NOT_FOUND' });
  });
  ...
  it('rejects unsupported networks before requesting entropy', async () => {
    ...
    ).rejects.toThrow('supports regtest, mutinynet, and mainnet only');
    expect(request).not.toHaveBeenCalledWith(expect.objectContaining({ method: 'snap_getBip32Entropy' }));
    expect(request).not.toHaveBeenCalledWith(expect.objectContaining({ method: 'snap_dialog' }));
  });
  ...
  it('validates signPsbt params before requesting entropy', async () => {
    ...
    await expect(
      handleRpcRequest(ORIGIN, {
        ...
        params: { network: 'mutinynet', psbt: null, signInputs: {} },
      }),
  ...
  it('rejects message signing for unknown addresses', async () => {
    ...
    await expect(
      handleRpcRequest(ORIGIN, {
        ...
        params: { network: 'mutinynet', address: 'tb1qunknown', message: 'hello' },
      }),
  ...
  it('renders arbitrary message signing content as copyable confirmation data', async () => {
    ...
    const result = await handleRpcRequest(ORIGIN, {
      method: 'ducat_signMessage',
      params: { network: 'mutinynet', address: keySet.record.sats.address, message },
    });
  ...
  it('rejects overlong messages before requesting entropy', async () => {
    ...
    await expect(
      handleRpcRequest(ORIGIN, {
        method: 'ducat_signMessage',
        params: { network: 'mutinynet', address: keySet.record.sats.address, message: 'x'.repeat(801) },
      }),
    ).rejects.toMatchObject({
      code: 'INVALID_PARAMS',
  ...
  it('rejects malformed PSBTs', async () => {
    ...
    await expect(
      handleRpcRequest(ORIGIN, {
        method: 'ducat_signPsbt',
        params: { network: 'mutinynet', psbt: 'not-a-psbt', signInputs: { [keySet.record.sats.address]: [0] } },
      }),
  ...
  it('signs PSBTs with the active imported account without an account identifier', async () => {
    ...
    expect(invalidateWalletInventory).toHaveBeenCalledWith('mutinynet');
    expect(request).not.toHaveBeenCalledWith(expect.objectContaining({ method: 'snap_getBip32Entropy' }));
    expect(request).toHaveBeenCalledWith(expect.objectContaining({ method: 'snap_dialog' }));
  });
  ...
  it('rejects PSBT signing when the confirmation is declined', async () => {
    ...
    await expect(
      handleRpcRequest(ORIGIN, {
        method: 'ducat_signPsbt',
        params: { network: 'mutinynet', psbt, signInputs: { [keySet.record.sats.address]: [0] } },
      }),
    ).rejects.toMatchObject({ code: 'USER_REJECTED' });
  });

  it('signs sats, UNIT, and vault inputs with one approval and signatures bound to the whole transaction', async () => {
    const request = setSnapMock(true);
    const fixture = makeThreeRolePsbt();
    const result = await handleRpcRequest(ORIGIN, {
      method: 'ducat_signPsbt',
      params: {
        network: 'mutinynet',
        psbt: fixture.psbt.toBase64(),
        signInputs: fixture.signInputs,
      },
    }) as { psbt: string };
    const signed = Psbt.fromBase64(result.psbt, { network: bitcoinNetwork('signet') });

    expect(request.mock.calls.filter(([arg]) => arg.method === 'snap_dialog')).toHaveLength(1);
    expect(signed.data.inputs[0].partialSig).toHaveLength(1);
    expect(signed.data.inputs[1].tapKeySig).toBeDefined();
    expect(signed.data.inputs[2].tapScriptSig).toHaveLength(1);
    expect([0, 1, 2].map((index) => signed.validateSignaturesOfInput(index, signatureValidator))).toEqual([true, true, true]);

    const changedOutput = copyThreeRoleSignatures(makeThreeRolePsbt({ outputValue: 88_999 }).psbt, signed);
    expect([0, 1, 2].map((index) => changedOutput.validateSignaturesOfInput(index, signatureValidator))).toEqual([false, false, false]);

    const changedPrevout = copyThreeRoleSignatures(makeThreeRolePsbt({ runesValue: 30_001 }).psbt, signed);
    expect([1, 2].map((index) => changedPrevout.validateSignaturesOfInput(index, signatureValidator))).toEqual([false, false]);

    const rejectedRequest = setSnapMock(false);
    await expect(handleRpcRequest(ORIGIN, {
      method: 'ducat_signPsbt',
      params: {
        network: 'mutinynet',
    ...
    expect(rejectedRequest.mock.calls.filter(([arg]) => arg.method === 'snap_dialog')).toHaveLength(1);
  });
  ...
  it('rejects unknown PSBT input indexes before showing confirmation', async () => {
    ...
    await expect(
      handleRpcRequest(ORIGIN, {
        method: 'ducat_signPsbt',
        params: { network: 'mutinynet', psbt, signInputs: { [keySet.record.sats.address]: [1] } },
      }),
    ).rejects.toMatchObject({
  ...
  it('rejects duplicate requested PSBT input indexes before requesting entropy', async () => {
    ...
    await expect(
      handleRpcRequest(ORIGIN, {
        method: 'ducat_signPsbt',
        params: { network: 'mutinynet', psbt, signInputs: { [keySet.record.sats.address]: [0, 0] } },
      }),
    ).rejects.toMatchObject({
      code: 'INVALID_PARAMS',
  ...
  it('rejects oversized PSBT signing requests before requesting entropy', async () => {
    ...
    await expect(
      handleRpcRequest(ORIGIN, {
        method: 'ducat_signPsbt',
        params: { network: 'mutinynet', psbt, signInputs: { [keySet.record.sats.address]: Array.from({ length: 81 }, (_, index) => index) } },
      }),
    ).rejects.toMatchObject({
      code: 'INVALID_PARAMS',
  ...
  it('shows every external recipient when a PSBT has more than the former visible fold', async () => {
    ...
    for (let index = 0; index < 9; index++) {
      const external = deriveAccountSetFromBaseNodes('mutinynet', testNode(20 + index), testNode(40 + index));
      psbt.addOutput({ address: external.record.sats.address, value: 50_000 });
    }

    await handleRpcRequest(ORIGIN, {
      method: 'ducat_signPsbt',
      params: { network: 'mutinynet', psbt: psbt.toBase64(), signInputs: { [keySet.record.sats.address]: [0] } },
    });
  ...
  it('shows an external recipient after leading change outputs', async () => {
    ...
    const external = deriveAccountSetFromBaseNodes('mutinynet', testNode(31), testNode(32));
    psbt.addOutput({ address: external.record.sats.address, value: 50_000 });
  ...
  it('shows every non-data output in the exact Admin reserve-issuance shape', async () => {
    const request = setSnapMock();
    const keySet = testKeySet();
    const guardian = deriveAccountSetFromBaseNodes('mutinynet', testNode(61), testNode(62));
    const psbt = new Psbt({ network: bitcoinNetwork('signet') });
  ...
  it('validates and measures the complete 120-output confirmation tree', async () => {
    const request = setSnapMock();
    const external = deriveAccountSetFromBaseNodes('mutinynet', testNode(65), testNode(66));
    const fixture = makeOutputBoundPsbt(120, 65, external.record.runes.address);

    await handleRpcRequest(ORIGIN, {
      method: 'ducat_signPsbt',
      params: { network: 'mutinynet', psbt: fixture.psbt, signInputs: { [fixture.keySet.record.sats.address]: [0] } },
    });
  ...
  it('retains the 121-output parser rejection before confirmation', async () => {
    const request = setSnapMock();
    const external = deriveAccountSetFromBaseNodes('mutinynet', testNode(67), testNode(68));
  ...
  it('itemizes recipients in batch entries seven through ten', async () => {
    ...
    const entries = Array.from({ length: 10 }, (_, index) => {
      const external = deriveAccountSetFromBaseNodes('mutinynet', testNode(70 + index), testNode(90 + index));
      const psbt = new Psbt({ network: bitcoinNetwork('signet') });
    ...
    await handleRpcRequest(ORIGIN, {
      method: 'ducat_signBatch',
      params: { network: 'mutinynet', entries },
    });
  ...
  it('classifies interleaved batch outputs without hiding external identities', async () => {
    const request = setSnapMock();
    const keySet = testKeySet();
    const external = deriveAccountSetFromBaseNodes('mutinynet', testNode(101), testNode(102));
  ...
  it('validates and measures every destination in a ten-by-120 batch tree', async () => {
    ...
    const recipients: string[] = [];
    const entries = Array.from({ length: 10 }, (_, index) => {
      const external = deriveAccountSetFromBaseNodes('mutinynet', testNode(110 + index), testNode(130 + index));
    ...
    await handleRpcRequest(ORIGIN, { method: 'ducat_signBatch', params: { network: 'mutinynet', entries } });
  ...
  it('renders Snap Home from the last connected network and origin', async () => {
    ...
    try {
      ...
      expect(rendered).toContain('mutinynet');
      ...
      expect(rendered).toContain('https://validator-mutinynet.dev.ducatprotocol.com');
      expect(rendered).toContain('manage-validator');
      expect(rendered).toContain('https://mutinynet.com/api');
