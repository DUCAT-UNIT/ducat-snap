...
describe('explicit Snap network selection', () => {
  it('returns the selected network without side effects', async () => {
    const { request } = setSnapMock({ selectedNetwork: 'mutinynet' });
  ...
  it('returns a same-network switch as a confirmation-free no-op', async () => {
    ...
    })).resolves.toEqual({ network: 'mutinynet', changed: false });
  ...
  it('persists an approved switch after showing Snap-owned network facts', async () => {
    ...
    expect(dialogText(request)).toContain('Mainnet (Alpha contract)');
    expect(dialogText(request)).toContain('https://validator-mainnet.alpha.ducatprotocol.com');
  ...
  it.each([
    ...
    {},
    { network: 'unknown' },
    { network: 'mutinynet', validator_base_url: 'https://attacker.example' },
  ...
  it.each([
    ...
    const { request, state } = setSnapMock({ selectedNetwork: 'mainnet' });
    ...
    try {
      await expect(handleRpcRequest(ORIGIN, {
        ...
        details: { selectedNetwork: 'mainnet', requestedNetwork: 'mutinynet' },
      });
    } finally {
