...
describe('compiled Snap artifact policy', () => {
  it('defines the exact production authority', () => {
    ...
    expect(artifactPolicy()).toEqual({
      ...
      allowed_deployments: ['mainnet', 'mutinynet'],
  ...
  it('defines development authority without inheriting production origins', () => {
    ...
    expect(artifactPolicy()).toEqual({
      ...
      allowed_deployments: ['regtest', 'mainnet', 'mutinynet'],
      default_deployment: 'mutinynet',
      debug_enabled: true,
  ...
  it.each([
    ...
    const { assertDeploymentAvailable } = loadPolicy({
      ...
      DUCAT_SNAP_DEV_ORIGINS: policy === 'development' ? DEVELOPMENT_ORIGINS : undefined,
    });

    expect(() => assertDeploymentAvailable(deployment as DeploymentId)).toThrow(expect.objectContaining({
      code: 'DEPLOYMENT_NOT_AVAILABLE',
    }));
  });

  it('development admits exactly the three canonical deployments', () => {
    const { assertDeploymentAvailable } = loadPolicy({
      DUCAT_SNAP_ARTIFACT_POLICY: 'development',
      DUCAT_SNAP_DEV_ORIGINS: DEVELOPMENT_ORIGINS,
    });

    for (const deployment of ['regtest', 'mainnet', 'mutinynet'] satisfies DeploymentId[]) {
