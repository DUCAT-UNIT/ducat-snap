...
describe('Snap state', () => {
  ...
  it('restores a valid last-network selection once without replacing unrelated state', async () => {
    ...
    expect(first.selectedNetwork).toBe('mutinynet');
    expect(second).toEqual(first);
    expect(first.keyOverrides).toEqual([keyOverride]);
    expect(first.lastOrigin).toBe('https://app.ducatprotocol.com');
    expect(first.networkEndpointOverrides?.mutinynet?.validator_base_url).toBe('https://validator-override.example');
    expect(getStoredState()).toHaveProperty('lastNetwork', 'mutinynet');
  ...
  it('prefers a valid explicit selection over conflicting legacy state', async () => {
    ...
    expect(getStoredState()).toEqual({ recentActions: [], selectedNetwork: 'mutinynet', lastNetwork: 'mainnet' });
  });
  ...
  it('hard-cuts removed selections, actions, keys, and endpoint overrides in development state', async () => {
    const developmentState = loadDevelopmentState();
    const { getStoredState } = setStateMock({
      recentActions: [
        { id: 'alpha', actionType: 'deposit', network: 'alpha-mainnet', origin: 'https://app.ducatprotocol.com', timestamp: 3 },
        { id: 'signet', actionType: 'deposit', network: 'signet', origin: 'https://app.ducatprotocol.com', timestamp: 2 },
        { id: 'testnet4', actionType: 'deposit', network: 'testnet4', origin: 'https://app.ducatprotocol.com', timestamp: 1 },
      ],
      keyOverrides: [
        { ...keyOverride, id: 'alpha-key', network: 'alpha-mainnet' },
        { ...keyOverride, id: 'signet-key', network: 'signet' },
        { ...keyOverride, id: 'testnet4-key', network: 'testnet4' },
      ],
      networkEndpointOverrides: {
        'alpha-mainnet': { validator_base_url: 'https://alpha.example', network_identity_verified: true },
        signet: { validator_base_url: 'https://signet.example', network_identity_verified: true },
        testnet4: { validator_base_url: 'https://testnet4.example', network_identity_verified: true },
      },
      selectedNetwork: 'alpha-mainnet',
    });

    await expect(developmentState.getState()).resolves.toEqual({
      recentActions: [],
      selectedNetwork: 'mutinynet',
    });
    expect(getStoredState()).toMatchObject({ selectedNetwork: 'mutinynet' });
  });
  ...
  it('filters corrupt actions, sorts newest first, caps history, and preserves session metadata', async () => {
    const validActions = Array.from({ length: 14 }, (_, index) => ({
      id: `valid-${index}`,
      actionType: 'deposit',
      network: index % 2 === 0 ? 'mutinynet' : 'mainnet',
    ...
    setStateMock({
      recentActions: [
        ...
        { id: 'bad-details', actionType: 'deposit', network: 'mutinynet', origin: 'http://localhost:3002', timestamp: 999, details: [] },
        ...validActions,
      ],
      selectedNetwork: 'mutinynet',
  ...
  it('preserves a newer network and key override when a stale session write finishes last', async () => {
    let managedState: DucatSnapState = { recentActions: [], selectedNetwork: 'mutinynet' };
