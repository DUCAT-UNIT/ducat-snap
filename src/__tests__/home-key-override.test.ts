...
describe('Snap Home key override content', () => {
  ...
  it('uses the imported override as the direct custody key instead of deriving child accounts', async () => {
    ...
    const { request } = setSnapStateMock({ recentActions: [], keyOverrides: [override], selectedNetwork: 'signet' });

    const keySet = await getActiveAccountKeySet('signet');
...
describe('Snap Home key override input handling', () => {
  it('does not let a stale Home form mutate another network', async () => {
    const override = keyOverride(9);
    const { interfaces, state } = setSnapStateMock({ recentActions: [], keyOverrides: [override], selectedNetwork: 'signet' });
    ...
    expect(JSON.stringify(interfaces.get('interface-1'))).toContain('signet');
  });
  ...
  it('saves a private key override from Snap Home without labels and clears submitted key material from rendered UI', async () => {
    ...
    const { interfaces, state } = setSnapStateMock({ recentActions: [], keyOverrides: [oldOverride], selectedNetwork: 'signet' });

    await handleHomeUserInput({
      id: 'interface-1',
        context: { screen: 'key-override', network: 'signet' },
      event: {
        type: UserInputEventType.FormSubmitEvent,
        name: 'import-private-key',
        value: {
  ...
  it('removes the imported override from Snap Home and restores derived account state', async () => {
    const override = keyOverride(9);
    const { state } = setSnapStateMock({ recentActions: [], keyOverrides: [override], selectedNetwork: 'signet' });
