...
function account(): SelectedAccountKeySet {
  return {
    ...deriveAccountSetFromBaseNodes('mutinynet', node(1), node(2)),
    id: 'derived:signet:0',
    source: 'derived',
    network: 'mutinynet',
  };
...
describe('WalletInventoryService', () => {
  it('returns the exact singular wallet snapshot and preserves decimal UNIT totals', async () => {
    ...
    const inventory = await service.get('mutinynet');
  ...
  it('deduplicates concurrent refreshes, caches for 30 seconds, and never serves expired data', async () => {
    ...
    const [first, parallel] = await Promise.all([service.get('mutinynet'), service.get('mutinynet')]);
    expect(first).toBe(parallel);
    expect(fetchImpl).toHaveBeenCalledTimes(3);
    time = 30_999;
    expect(await service.get('mutinynet')).toBe(first);
    expect(fetchImpl).toHaveBeenCalledTimes(3);
    time = 31_000;
    expect(await service.get('mutinynet')).not.toBe(first);
    expect(fetchImpl).toHaveBeenCalledTimes(6);
  });

  it('invalidates a network snapshot explicitly', async () => {
    const { fetchImpl, service } = fixture(() => 1_000);
    await service.get('mutinynet');
    service.invalidate('mutinynet');
    await service.get('mutinynet');
    expect(fetchImpl).toHaveBeenCalledTimes(6);
  });
  ...
  it('sanitizes endpoint failures', async () => {
    ...
    const service = new WalletInventoryService({
      ...
      resolveAccount: async () => active,
      resolveProfile: async () => ({
        id: 'mutinynet', label: 'Signet', bitcoin_network: 'signet', expected_validator_chain_network: 'mutiny', validator_base_url: 'https://bad.example', esplora_base_url: 'https://bad.example',
      }),
      verifyEndpoint: async () => { throw new Error('secret https://bad.example/token'); },
    });
    await expect(service.get('mutinynet')).rejects.toMatchObject({
      code: 'WALLET_DATA_UNAVAILABLE',
      message: 'Wallet data is temporarily unavailable.',
    });
    await expect(service.get('mutinynet')).rejects.not.toThrow(/bad\.example|secret|token/iu);
  });
});
