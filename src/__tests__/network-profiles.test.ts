import {
  effectiveNetworkProfile,
  networkProfile,
  networkProfiles,
  normalizeNetworkEndpointUrl,
  validateNetworkProfiles,
} from '../network-profiles';
import { verifyDeploymentEndpointIdentity } from '../network-endpoint-policy';
import {
  bitcoinNetworkForDeployment,
  DUCAT_GUARDIAN_PUBKEYS,
  guardianKeyPolicyReady,
  isKnownGuardianPubkey,
  normalizeDeploymentId,
} from '../networks';

describe('deployment and Bitcoin identity', () => {
  it('accepts only canonical wallet identities and documented aliases', () => {
    expect(normalizeDeploymentId('main')).toBe('mainnet');
    expect(normalizeDeploymentId('mutiny')).toBe('mutinynet');
    expect(bitcoinNetworkForDeployment('mainnet')).toBe('mainnet');
    expect(bitcoinNetworkForDeployment('mutinynet')).toBe('signet');
  });

  it.each([undefined, null, '', 'alpha', 'alpha-mainnet', 'signet', 'testnet4', 'unknown'])(
    'rejects removed or unknown deployment identifiers: %p',
    (value) => {
    expect(() => normalizeDeploymentId(value)).toThrow();
    },
  );
});

describe('network profiles', () => {
  it('carries an explicit, correct Bitcoin network for every bundled deployment', () => {
    expect(networkProfiles().map((profile) => profile.id)).toEqual(['mainnet', 'mutinynet']);
    expect(networkProfiles().map((profile) => [profile.id, profile.expected_validator_chain_network])).toEqual([
      ['mainnet', 'alpha-mainnet'],
      ['mutinynet', 'mutiny'],
    ]);
    for (const profile of networkProfiles()) {
      expect(profile.bitcoin_network).toBe(bitcoinNetworkForDeployment(profile.id));
    }
  });

  it('loads one bundled profile per supported network', () => {
    const profiles = validateNetworkProfiles({
      networks: [
        {
          id: 'mutinynet',
          label: 'Mutinynet',
          bitcoin_network: 'signet',
          expected_validator_chain_network: 'mutiny',
          validator_base_url: 'https://validator-mutinynet.dev.ducatprotocol.com',
          esplora_base_url: 'https://mutinynet.com/api',
        },
      ],
    });

    expect(profiles).toEqual([
      expect.objectContaining({
        id: 'mutinynet',
        bitcoin_network: 'signet',
        expected_validator_chain_network: 'mutiny',
        validator_base_url: 'https://validator-mutinynet.dev.ducatprotocol.com',
        esplora_base_url: 'https://mutinynet.com/api',
      }),
    ]);
  });

  it('compiles exactly regtest, mainnet, and Mutinynet for development', () => {
    const previous = {
      policy: process.env.DUCAT_SNAP_ARTIFACT_POLICY,
      origins: process.env.DUCAT_SNAP_DEV_ORIGINS,
    };
    try {
      jest.isolateModules(() => {
        process.env.DUCAT_SNAP_ARTIFACT_POLICY = 'development';
        process.env.DUCAT_SNAP_DEV_ORIGINS = 'http://localhost:3000,http://localhost:8075,http://frontend:3000,http://ducat-admin:8075';
        const development = require('../network-profiles') as typeof import('../network-profiles');
        expect(development.networkProfiles().map((profile) => profile.id)).toEqual(['regtest', 'mainnet', 'mutinynet']);
        expect(development.networkProfile('mainnet')).toEqual({
          id: 'mainnet', label: 'Mainnet (Alpha contract)', bitcoin_network: 'mainnet',
          expected_validator_chain_network: 'alpha-mainnet',
          validator_base_url: 'https://validator-mainnet.alpha.ducatprotocol.com', esplora_base_url: 'https://mempool.space/api',
        });
        expect(development.networkProfile('regtest').expected_validator_chain_network).toBe('regtest');
      });
    } finally {
      for (const [name, value] of [
        ['DUCAT_SNAP_ARTIFACT_POLICY', previous.policy],
        ['DUCAT_SNAP_DEV_ORIGINS', previous.origins],
      ] as const) {
        if (value === undefined) delete process.env[name];
        else process.env[name] = value;
      }
    }
  });

  it('rejects duplicate network ids', () => {
    expect(() => validateNetworkProfiles({
      networks: [
        { id: 'mutinynet', label: 'A', bitcoin_network: 'signet', expected_validator_chain_network: 'mutiny', validator_base_url: 'https://validator-a.example', esplora_base_url: 'https://esplora-a.example' },
        { id: 'mutinynet', label: 'B', bitcoin_network: 'signet', expected_validator_chain_network: 'mutiny', validator_base_url: 'https://validator-b.example', esplora_base_url: 'https://esplora-b.example' },
      ],
    })).toThrow('duplicate network profile: mutinynet');
  });

  it('rejects absent or inconsistent Bitcoin-network mappings', () => {
    expect(() => validateNetworkProfiles({
      networks: [
        { id: 'mutinynet', label: 'Mutinynet', expected_validator_chain_network: 'mutiny', validator_base_url: 'https://validator.example', esplora_base_url: 'https://esplora.example' },
      ],
    })).toThrow('invalid Bitcoin network');

    expect(() => validateNetworkProfiles({
      networks: [
        { id: 'mutinynet', label: 'Mutinynet', bitcoin_network: 'mainnet', expected_validator_chain_network: 'mutiny', validator_base_url: 'https://validator.example', esplora_base_url: 'https://esplora.example' },
      ],
    })).toThrow('network profile mutinynet must map to Bitcoin signet');
  });

  it('rejects malformed profile containers and entries', () => {
    expect(() => validateNetworkProfiles(null)).toThrow('network profiles must include a networks array');
    expect(() => validateNetworkProfiles({ networks: [null] })).toThrow('network profile entries must be objects');
    expect(() => validateNetworkProfiles({ networks: ['signet'] })).toThrow('network profile entries must be objects');
  });

  it('rejects non-http endpoint URLs', () => {
    expect(() => validateNetworkProfiles({
      networks: [
        { id: 'mutinynet', label: 'Mutinynet', bitcoin_network: 'signet', expected_validator_chain_network: 'mutiny', validator_base_url: 'file:///tmp/validator', esplora_base_url: 'https://mutinynet.com/api' },
      ],
    })).toThrow('validator_base_url must be an HTTP(S) URL');
  });

  it('resolves bundled endpoints for a network', () => {
    const profile = networkProfile('mainnet');

    expect(profile.validator_base_url).toBe('https://validator-mainnet.alpha.ducatprotocol.com');
    expect(profile.esplora_base_url).toBe('https://mempool.space/api');
  });

  it('applies explicit overrides over bundled endpoints', () => {
    const profile = effectiveNetworkProfile('mainnet', {
      mainnet: {
        validator_base_url: 'https://validator-override.example',
        esplora_base_url: 'https://esplora-override.example/api',
      },
    });

    expect(profile.validator_base_url).toBe('https://validator-override.example');
    expect(profile.esplora_base_url).toBe('https://esplora-override.example/api');
    expect(profile.expected_validator_chain_network).toBe('alpha-mainnet');
  });

  it('allows plaintext HTTP only for regtest loopback endpoints', () => {
    expect(() => normalizeNetworkEndpointUrl(
      'http://validator.example',
      'validator_base_url',
      'signet',
    )).toThrow('validator_base_url must use HTTPS outside regtest loopback development');
    expect(() => normalizeNetworkEndpointUrl(
      'http://validator.example',
      'validator_base_url',
      'regtest',
    )).toThrow('validator_base_url must use HTTPS outside regtest loopback development');

    expect(normalizeNetworkEndpointUrl(
      'http://127.0.0.1:8083/',
      'validator_base_url',
      'regtest',
    )).toBe('http://127.0.0.1:8083');
  });

  it('verifies validator and Esplora network identity', async () => {
    await expect(verifyDeploymentEndpointIdentity('mutiny', 'signet', 'validator', 'https://validator.example',
      jest.fn(async () => Response.json([])) as typeof fetch,
    )).rejects.toThrow('validator endpoint did not report mutiny');

    await expect(verifyDeploymentEndpointIdentity('mutiny', 'signet', 'validator', 'https://validator.example',
      jest.fn(async () => Response.json({ chain_network: 'testnet4' })) as typeof fetch,
    )).rejects.toThrow('validator endpoint did not report mutiny');

    await expect(verifyDeploymentEndpointIdentity('mutiny', 'signet', 'esplora', 'https://esplora.example',
      jest.fn(async () => new Response('00000000da84f2bafbbc53dee25a72ae507ff4914b867c565be350b0da8bf043')) as typeof fetch,
    )).rejects.toThrow('esplora endpoint is not on signet');

    await expect(verifyDeploymentEndpointIdentity('mutiny', 'signet', 'esplora', 'https://esplora.example',
      jest.fn(async () => new Response('00000008819873e925422c1ff0f99f7cc9bbb232af63a077a480a3633bee1ef6')) as typeof fetch,
    )).resolves.toBeUndefined();
  });

  it('requires the exact profile validator identity while mapping Esplora to Bitcoin genesis', async () => {
    const mutinyValidator = jest.fn(async () => Response.json({ chain_network: 'mutiny' })) as typeof fetch;
    const alphaValidator = jest.fn(async () => Response.json({ chain_network: 'alpha-mainnet' })) as typeof fetch;

    await expect(verifyDeploymentEndpointIdentity(
      'mutiny',
      'signet',
      'validator',
      'https://validator.example',
      mutinyValidator,
    )).resolves.toBeUndefined();

    await expect(verifyDeploymentEndpointIdentity(
      'mainnet',
      'signet',
      'validator',
      'https://validator.example',
      mutinyValidator,
    )).rejects.toThrow('validator endpoint did not report mainnet');

    await expect(verifyDeploymentEndpointIdentity(
      'alpha-mainnet',
      'mainnet',
      'validator',
      'https://validator.example',
      alphaValidator,
    )).resolves.toBeUndefined();

    await expect(verifyDeploymentEndpointIdentity(
      'mainnet',
      'mainnet',
      'validator',
      'https://validator.example',
      alphaValidator,
    )).rejects.toThrow('validator endpoint did not report mainnet');

    await expect(verifyDeploymentEndpointIdentity(
      'alpha-mainnet',
      'mainnet',
      'esplora',
      'https://esplora.example',
      jest.fn(async () => new Response('000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f')) as typeof fetch,
    )).resolves.toBeUndefined();
  });

  it('supports mainnet contract promotion through profile expectation only', async () => {
    const promoted = {
      ...networkProfile('mainnet'),
      expected_validator_chain_network: 'mainnet',
      validator_base_url: 'https://validator-mainnet.example',
    };
    const fetchMainnet = jest.fn(async () => Response.json({ chain_network: 'mainnet' })) as typeof fetch;

    await expect(verifyDeploymentEndpointIdentity(
      promoted.expected_validator_chain_network,
      promoted.bitcoin_network,
      'validator',
      promoted.validator_base_url,
      fetchMainnet,
    )).resolves.toBeUndefined();
    expect(promoted.id).toBe('mainnet');
  });

  it('keeps guardian-backed mainnet signing closed', () => {
    const mutinynetGuardian = DUCAT_GUARDIAN_PUBKEYS.mutinynet[0];
    expect(guardianKeyPolicyReady('mainnet')).toBe(false);
    expect(isKnownGuardianPubkey('mainnet', mutinynetGuardian)).toBe(false);
    expect(guardianKeyPolicyReady('mutinynet')).toBe(true);
    expect(isKnownGuardianPubkey('mutinynet', mutinynetGuardian)).toBe(true);
  });
});
