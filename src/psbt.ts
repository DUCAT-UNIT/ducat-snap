...
const SEQUENCE_TIMELOCK_TYPE = 0x00400000;
const SEQUENCE_TIMELOCK_MASK = 0x0000ffff;
...
type TaprootScriptPathOwnership = {
  ...
  /** Block delay committed by an owned BitVM3 timeout leaf. */
...
function checkOwnedTaprootScriptPathInput(
  ...
  // actually commit to).
  const ownedTimeoutLeaf = input.tapLeafScript
    .map((tapLeaf) => ({
      match: matchTimeoutLeafHex(Buffer.from(tapLeaf.script).toString('hex')),
      tapLeaf,
    }))
    .find(({ match, tapLeaf }) => match?.operator === vaultPubkeyHex && tapLeafCommitsToOutputKey(tapLeaf, outputKey));

  if (ownedTimeoutLeaf?.match) {
    return {
      ok: true,
      reason: 'committed BitVM3 timeout leaf',
      leafHash: tapLeafHashForSigning(ownedTimeoutLeaf.tapLeaf),
      leafKind: 'bitvm3-timeout',
      timeoutWindow: ownedTimeoutLeaf.match.window,
    };
  }
...
function assertBitvm3TimeoutPolicy(psbt: Psbt, index: number, window: number): void {
  const sequence = psbt.txInputs[index]?.sequence;
  let reason: string | null = null;

  if (psbt.version < 2) {
    reason = 'transaction version is below 2';
  } else if (sequence === undefined) {
    reason = 'input sequence is unavailable';
  } else if ((sequence & SEQUENCE_TIMELOCK_DISABLE) !== 0) {
    reason = 'input sequence disables relative lock-time';
  } else if ((sequence & SEQUENCE_TIMELOCK_TYPE) !== 0) {
    reason = 'input sequence uses time units instead of the committed block units';
  } else if ((sequence & SEQUENCE_TIMELOCK_MASK) < window) {
    reason = 'input sequence is below the committed block delay';
  }

  if (reason) {
    throw ducatError(
      'PSBT_TIMEOUT_POLICY_INVALID',
      'This BitVM3 timeout input does not satisfy its committed relative timelock and will not be signed.',
      {
        inputIndex: index,
        reason,
        transactionVersion: psbt.version,
        inputSequence: sequence,
        timeoutWindowBlocks: window,
      },
    );
  }
}
...
function validateSignedInput(psbt: Psbt, index: number, address: string, keySet: AccountPublicSet): PsbtInputSummary {
  ...
  if (!sameScript(inputScript, expectedScript)) {
    ...
    if (scriptPathOwnership?.ok) {
      if (scriptPathOwnership.leafKind === 'bitvm3-timeout' && scriptPathOwnership.timeoutWindow !== undefined) {
        assertBitvm3TimeoutPolicy(psbt, index, scriptPathOwnership.timeoutWindow);
      }
